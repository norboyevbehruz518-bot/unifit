...
## 2026-06-13 — Profile wizard (5 steps) + `/app/profile` summary

- **Done:** Five new UI primitives (Slider, SegmentedControl, RadioCardGroup, SearchableMultiSelect, StepProgress), all added to `/dev/design-system`. Built the profile-creation wizard (`src/components/features/profile-wizard/`): Step 1 Academics (GPA/scale, SAT/ACT, English test), Step 2 Direction (1–3 ranked majors via `SearchableMultiSelect`), Step 3 Money (budget slider, aid-need level), Step 4 Profile-strength rubric (4 questions, answer-level storage per DOMAIN.md §2), Step 5 Review + Supabase save (`src/lib/profile-wizard/submit.ts`, upsert by `id`). Draft autosaves to `localStorage` so progress survives a refresh. Wired `/app/profile`: redirects to `/app/setup` if no profile row exists yet, otherwise renders a read-only summary with an Edit link back into the wizard. Suite now 907/907 (unchanged net — wizard logic covered via existing fit-engine/format helpers), typecheck and lint clean.
- **Next:** Phase 5 — university selection + fit results (the core experience): data-access layer, university picker, results page with the list-balance hero.

## 2026-06-14 — Phase 5 complete: university selection + fit results

- **Done:** The core experience. (1) Data-access layer (`src/lib/data/`): snake_case→camelCase row mappers for `universities`, `profiles`, `saved_lists`/`list_items`, and append-only `fit_snapshots`, with round-trip unit tests against the seed-data shape. (2) University picker (`src/components/features/university-picker/`): searchable, tier-filterable (`SegmentedControl` over `TIER_BOUNDS`) list capped at 12 selections, with the "strong lists mix Reach, Target and Safety schools" tip firing when ≥2 schools are selected and ≥75% are Tier 1 (covers "mostly Reaches with one Target", not just pure-Reach lists — unit tested). (3) `/app/universities`: server-loaded selection page; "Save & see my fit" runs a server action (`saveUniversitySelection`) that replaces the list and redirects to results. (4) Snapshot orchestration (`src/lib/results/`): `getOrComputeResults` computes-and-freezes a `fit_snapshot` (with `algorithm_version`) the first time a university is seen, and surfaces `needsRecalculate` when an existing snapshot's profile/algorithm version is stale; `recalculateResults` is the explicit server action that re-freezes everything. (5) `/app/results`: the list-balance summary (`analyzeListBalance`) is the hero — large, color-coded, advisory sentence front and center — followed by Safety/Target/Reach sections (toggleable order) with category badges, overall score, three `ScoreBar`s, expandable "Why this score?" explanations, and a data-confidence badge; gated universities get their own "Action needed" section listing what would unlock them; empty-list and low-confidence states handled inline, profile-incomplete redirects to `/app/setup`. (6) Added Profile/Universities/Results nav to the `/app` header. Architecture keeps client JS to small islands (picker, results-list toggle, recalculate button) with all data fetching and fit-score computation server-side, for the <2s mobile-interactive target. Suite now 918/918, typecheck/lint/production-build all clean.
- **Next:** Live walkthrough against the hosted Supabase project (login → select → tip → save → results → recalculate) with a real account. Otherwise Phase 5 is feature-complete; future work is polish/iteration on the core experience.

## 2026-06-16 — Sprint 3 complete: security, CI/CD, production fixes, AP scores, competitor ranking
